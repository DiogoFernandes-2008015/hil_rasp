...
def des_traj(t):
    ...
    x1p = 0.25*np.pi*np.sin(np.pi*t)
    x2p = 0.25*np.pi*np.cos(np.pi*t)
    phip = np.pi/24*np.cos((np.pi / 24) * t)
    return np.array([x1, x2, phi]), np.array([x1p, x2p, phip]) 
...
try:
    while sim_time < t_simulation:
        ...
        xd, xdp = des_traj(sim_time)
        
        xd1_h.append(xd[0])
        xd2_h.append(xd[1])
        xd3_h.append(xd[2])
        
        msg = f"{xd[0]};{xd[1]};{xd[2]};{xdp[0]};{xdp[1]};{xdp[2]}".encode()
    ...
    dados_estimados = [xd1hh, xd2hh, xd3hh]
    
    for i in range(3):
        plt.subplot(3, 1, i+1)
        plt.plot(dados_desejados[i], label='Desejado', color='blue', linestyle='--')
        plt.plot(dados_estimados[i], label='Estimado', color='green')
    ...
    plt.plot(xd1_h, xd2_h, label='Trajetória Desejada', color='blue', linewidth=1)
    plt.plot(xd1hh, xd2hh, label='Trajetória Executada (Pi)', color='red',linestyle='--', linewidth=1.5)
